 ...
 * @retval int
 */
UINT MX_NetXDuo_TcpServer_Init(VOID *memory_ptr) {
	UINT ret = NX_SUCCESS;
	...
	if (nx_packet_pool_create(&NxAppPool, "NetXDuo App Pool", DEFAULT_PAYLOAD_SIZE, pointer, NX_APP_PACKET_POOL_SIZE) != NX_SUCCESS)
	...
	ret = nx_ip_create(&NetXDuoEthIpInstance, "NetX Ip instance", NX_APP_DEFAULT_IP_ADDRESS, NX_APP_DEFAULT_NET_MASK, &NxAppPool,
			nx_stm32_eth_driver, pointer, Nx_IP_INSTANCE_THREAD_SIZE, NX_APP_INSTANCE_PRIORITY);
	if (ret != NX_SUCCESS)
		return NX_NOT_SUCCESSFUL;

	// Allocate the memory for ARP
	if (tx_byte_allocate(byte_pool, (VOID**) &pointer, DEFAULT_ARP_CACHE_SIZE,
	...
	if (tx_byte_allocate(byte_pool, (VOID**) &pointer,
			2 * DEFAULT_ARP_CACHE_SIZE, TX_NO_WAIT) != TX_SUCCESS) {
		return TX_POOL_ERROR;
	}

	/* create the TCP server thread */
	ret = tx_thread_create(&AppTCPThread, "App TCP Thread",
			App_TCP_Thread_Entry, 0, pointer, NX_APP_THREAD_STACK_SIZE,
			NX_APP_THREAD_PRIORITY, NX_APP_THREAD_PRIORITY, TX_NO_TIME_SLICE,
			TX_DONT_START);
	...
	if (tx_byte_allocate(byte_pool, (VOID**) &pointer, NX_APP_THREAD_STACK_SIZE,
			TX_NO_WAIT) != TX_SUCCESS) {
		return TX_POOL_ERROR;
	}
	...
	ret = tx_thread_create(&NxAppThread, "NetXDuo App thread",
			nx_app_thread_entry, 0, pointer, NX_APP_THREAD_STACK_SIZE,
			NX_APP_THREAD_PRIORITY, NX_APP_THREAD_PRIORITY, TX_NO_TIME_SLICE,
			TX_AUTO_START);
	...
	ret = tx_thread_create(&AppLinkThread, "App Link Thread",
			App_Link_Thread_Entry, 0, pointer, NX_APP_THREAD_STACK_SIZE,
 ...
 */
static VOID tcp_listen_callback(NX_TCP_SOCKET *socket_ptr, UINT port) {
	tx_semaphore_put(&TCPListenSemaphore);
}
 ...
 * @retval none
 */
#define TCPListenSemaphore	DHCPSemaphore
...
static VOID App_TCP_Thread_Entry(ULONG thread_input) {
	...
	// TCP Server Listen 開始、新規クライアントの接続を開始する
	...
	// クライアント接続待機用セマフォを待機
	if (tx_semaphore_get(&TCPListenSemaphore, TX_WAIT_FOREVER) != TX_SUCCESS) {
		// クライアント接続待機用セマフォでエラー発生
		Error_Handler();
	} else {
		// データ交換を開始する前に、新しいクライアント接続を受け入れる
	...
	while (1) {
		...
		if (ret == NX_SUCCESS) {
			...
			if (ret == NX_SUCCESS) {
				...
				// 受信したUDPデータグラムから送信者のIPとポート番号を抽出する。
				nx_udp_source_extract(data_packet, &source_ip_address, &source_port);

				// 受信パケットをバッファにコピーする。受信バイト数はbytes_readに返される。
				// data_buffer[]は、受信パケットを保持するのに十分な大きさでなければならない。
			//	nx_packet_length_get (NX_PACKET *packet_ptr, ULONG *length);
				nx_packet_data_retrieve(data_packet, data_buffer, &bytes_read);
			//	nx_packet_data_extract_offset(data_packet, 0, data_buffer, sizeof(data_buffer), &bytes_read);
